...
async def init(bot):
    ...
    try:
        if len(bot.args)>0:
            ...
            try:
                if user is not None:
                    for admin in global_vars.botadmins:
                        if str(admin[0]) == str(user.id):
                            await bot.sendMessage( ":x: No.... just no!".format(user))
                            return
                    await bot.client.ban(user, delete_message_days=1)
                    #if not hasattr(user, 'display_name'):
                    await bot.sendMessage( ":white_check_mark:  Banned <@{0.id}> (**ID:** {0.id})".format(user))
