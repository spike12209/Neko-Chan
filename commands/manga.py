...
DESC="Shows informations about an manga"
USAGE="manga [name]"

async def init(bot):
    try:
        data = await tools.mal_manga("+".join(bot.args))
        print(data)
        if data is None or len(data)==0:
            bot.sendMessage("Sorry, no Manga named \"{}\" found :sob:".format(" ".join(bot.args)))
            return
        ...
        if len(root)==0:
            ...
            choose = "**Multiple manga found**\r\nChoose one of the following manga by sending its number:\r\n"
        ...
        msg += '**Link: ** http://myanimelist.net/manga/{}\r\n'.format(entry.find('id').text)
