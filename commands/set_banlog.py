...
import pprint
DESC="Set channel for ban bot.messages"
USAGE="set_banlog #channel"
...
async def init(bot):
    ...
    try:
        ...
        if bot.args[0] == "off":
            await config.setConf(bot.message.server.id, "ban_log_channel", None)
            ...
            if channel is not None:
                await config.setConf(bot.message.server.id, "ban_log_channel", channel.name)
                await bot.sendMessage( "Channel set!")
            else:
                await bot.sendMessage( "Such channel doesn't exist!")

        else:
            await bot.sendMessage("Not enough arguments! Current channel: {}").format(config.getConf(message.server.id, "ban_log_channel"))
    except Exception:
