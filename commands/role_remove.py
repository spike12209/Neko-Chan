...
DESC="Remove role of someone!"
USAGE="role_remove [role] [member]"
...
async def init(bot):
    ...
    try:
        ...
        bot.args.pop(0)
        if role is None:
            await bot.sendMessage( "`{}` is not a valid role. (Usage: role_remove [`role`] [`member`])".format(crole))
            return False
        if len(bot.args)>0:
        ...
        if user is not None:
            try:
                await bot.client.remove_roles(user, role)
                await bot.sendMessage( "Role `{}` has been removed from `{}`.".format(role, user))
            except discord.Forbidden:
                await bot.sendMessage( "This role is above bot role, i can't do that!")
        else:
            await bot.sendMessage( "`{}` is not a valid user! (Usage: role_remove [`role`] [`member`])".format(bot.args[0]))
            return False
    except Exception:
